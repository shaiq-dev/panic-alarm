import { CipherGCMTypes, randomBytes, createCipheriv, createDecipheriv } from "crypto";
...
export class Cypher {
  ...
  encrypt(value: string) {
    try {
      ...
      const encryptedValueBuffer = Buffer.concat([encryptedValue, cipher.final()]);
  ...
  decrypt(encryptedValue: string) {
    try {
      const authTag = Buffer.from(encryptedValue.slice(-(this.AUTH_TAG_BYTE_LEN * 2)), "hex");
      const iv = Buffer.from(encryptedValue.slice(0, this.IV_BYTE_LEN * 2), "hex");
      const encryptedValueDataPart = encryptedValue.slice(
        this.IV_BYTE_LEN * 2,
        -(this.AUTH_TAG_BYTE_LEN * 2)
      );

      const decipher = createDecipheriv(this.blockCipher, this.encryptionKey, iv, {
        authTagLength: this.AUTH_TAG_BYTE_LEN,
      });
